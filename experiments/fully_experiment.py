...
class FullyExperiment(BaseExperiment):
    ...
    def _generate_single_hop_operation(self, step: int) -> List[RoutingOperation]:
        ...
        if step == 1:
            ...
            return [RoutingOperation(OperationType.ADD_CONSTANT, val)]
        ...
        if mod == 0:
            ...
            return [RoutingOperation(OperationType.MULTIPLY_CONSTANT, val)]
        else:  # mod == 2
    ...
    def run_stress_suite(
        ...
        """
        Run stress tests (limit testing) for all fully homomorphic schemes.
        ...
        schemes_to_run = {}
        for name in self.fully_schemes:
            try:
        ...
        iterator = tqdm(range(total), desc="Fully Stress Test") if show_progress else range(total)
        run_idx = 0
