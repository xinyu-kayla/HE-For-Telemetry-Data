"""GSW fully homomorphic encryption for boolean plaintexts."""
...
class GSWScheme(HEScheme):
    ...
    def add(self, c1: np.ndarray, c2: np.ndarray, keypair: HEKeyPair) -> np.ndarray:
        q = keypair.public_key["q"]
        return (c1 + c2) % q

    def multiply(self, c1: np.ndarray, c2: np.ndarray, keypair: HEKeyPair) -> np.ndarray:
        q = keypair.public_key["q"]
        Ginv = self.g_inv(c2)
        return (c1 @ Ginv) % q

    def get_noise_budget(self, ciphertext) -> float:
        return float(np.sum(np.abs(ciphertext))) / (self.q * 1000.0)